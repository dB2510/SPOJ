...
void update(lli pos,lli v,lli tl,lli tr,lli value)
{
	if(tl==tr)
		tree[v]=make_data(value);
	else
	{
		lli mid=(tl+tr)/2;
		if(pos<=mid)
			update(pos,2*v,tl,mid,value);
		else
			update(pos,2*v+1,mid+1,tr,value);
		tree[v]=combine(tree[2*v],tree[2*v+1]);
	}
}
